...
export function RaceHeroCard({
  ...
  if (!race) {
    return (
      <div className="relative flex min-h-[360px] flex-col items-center justify-center overflow-hidden rounded-2xl border border-border bg-card/80 p-8 text-center">
        ...
        <Button
          type="button"
          className="mt-4 rounded-xl bg-primary hover:bg-primary/90 text-primary-foreground font-bold shadow-card shadow-primary/20"
          onClick={onRegister}
        >
    ...
    <div className="relative flex min-h-[360px] flex-col justify-end overflow-hidden rounded-2xl">
      <div className="absolute inset-0">
        {cover ? (
          ...
          <div className="flex h-full w-full items-center justify-center bg-card border-b border-border">
            <ImageOff className="size-10 text-on-surface-variant" />
          </div>
        )}
      ...
      <div className="relative flex w-full flex-col gap-4 p-6 md:p-8">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <span className="rounded-xl bg-primary px-3 py-1 text-xs font-bold text-primary-foreground shadow-sm shadow-primary/20">
            Próxima carrera
          </span>
          {race.location ? (
            <span className="flex items-center gap-1.5 rounded-lg bg-card/60 px-2.5 py-1 text-xs font-medium text-foreground backdrop-blur-md border border-border/50">
        ...
        <div className="flex flex-wrap gap-3">
          {COUNTDOWN_UNITS.map(({ key, label }) => (
            <div
              key={key}
              className="min-w-[76px] rounded-xl border border-border/50 bg-card/60 p-3 text-center backdrop-blur-md"
            >
