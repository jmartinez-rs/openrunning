...
import { Calendar, MapPin, Target, ChevronDown, ChevronUp, Link as LinkIcon } from "lucide-react"
    ...
    <div className="relative flex flex-col overflow-hidden rounded-3xl border border-orange-500/50 bg-slate-900 shadow-[0_0_40px_rgba(249,115,22,0.1)]">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-px bg-slate-800/50 border-t border-slate-800">
        <div className="bg-slate-900 p-4">
          <p className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Distancia</p>
          <p className="text-lg font-black text-white">{race.distance_km} KM</p>
        </div>
        <div className="bg-slate-900 p-4">
          <p className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Dorsal / Corral</p>
          <p className="text-lg font-black text-white truncate">
            {race.bib_number || "--"} / {meta.corral || "--"}
          </p>
        </div>
        <div className="bg-slate-900 p-4">
          <p className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Largada</p>
          <p className="text-lg font-black text-white">{meta.start_time || "--:--"}</p>
        </div>
        <div className="bg-slate-900 p-4">
          <p className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Estado</p>
          <div className="flex items-center gap-1.5 mt-1">
            <div className={`w-2 h-2 rounded-full ${meta.status === 'confirmed' ? 'bg-emerald-500' : 'bg-amber-500'}`} />
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-bold text-white tracking-tight">Estrategia y Objetivos</span>
        </div>
        ...
        <div className="p-6 bg-slate-950 border-t border-slate-800 flex flex-col gap-6 animate-in slide-in-from-top-2 duration-300">
          <div className="grid md:grid-cols-2 gap-6">
            <div className="space-y-4">
              <div>
                <p className="text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-1">Ritmo Objetivo</p>
                <p className="text-xl font-black text-white tabular-nums">{meta.target_pace || "--:--"} <span className="text-xs font-medium text-slate-400">/km</span></p>
              </div>
              <div>
                <p className="text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-1">Tiempo Estimado</p>
                <p className="text-xl font-black text-white tabular-nums">{meta.target_time || "--:--:--"}</p>
              </div>
            </div>
            
            <div className="space-y-4">
              <div>
                <p className="text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-2">Segmentación (Splits)</p>
                <div className="text-sm text-slate-300 whitespace-pre-wrap bg-slate-900 p-3 rounded-xl border border-slate-800 min-h-[60px]">
                  {meta.splits_strategy || "Aún no has definido tu estrategia de carrera."}
                </div>
              </div>
              <div>
                <p className="text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-2">Nutrición e Hidratación</p>
                <div className="text-sm text-slate-300 whitespace-pre-wrap bg-slate-900 p-3 rounded-xl border border-slate-800 min-h-[60px]">
          ...
          <div className="flex justify-end mt-2 pt-4 border-t border-slate-800/50">
            <Button asChild variant="outline" className="rounded-xl border-slate-700 bg-slate-900 text-slate-300 hover:text-white hover:bg-slate-800">
              <Link to="/routines">
                <LinkIcon className="mr-2 size-4" /> Ver plan de entrenamiento asociado
              </Link>
            </Button>
          </div>
        </div>
