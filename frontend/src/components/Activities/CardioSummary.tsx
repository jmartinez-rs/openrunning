  ...
  return (
    <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-4 sm:p-5 shadow-xl space-y-6">
      <div className="flex items-center gap-2 pb-1 border-b border-slate-800/80">
        ...
        <h2 className="text-sm font-bold text-white tracking-tight">Parciales y Zonas</h2>
      </div>
      ...
      {splits.length > 0 ? (
        <div className="space-y-3">
          <div className="flex items-center justify-between text-xs font-bold text-slate-400 uppercase tracking-wider">
            ...
            <span className="text-[11px] text-slate-500 font-normal">Total {splits.length} km</span>
          </div>
          ...
          <div className="overflow-hidden rounded-xl border border-slate-800/80 bg-slate-950/60">
            <table className="w-full text-xs">
              <thead className="bg-slate-900/90 text-left border-b border-slate-800/80">
                <tr>
                  <th className="px-3.5 py-2 font-bold text-slate-400 uppercase tracking-wider">Km</th>
                  <th className="px-3.5 py-2 font-bold text-slate-400 uppercase tracking-wider">Ritmo</th>
                  <th className="px-3.5 py-2 font-bold text-slate-400 uppercase tracking-wider text-right">Tiempo</th>
                </tr>
              </thead>
      ...
      {hasZones ? (
        <div className="space-y-3">
          <div className="flex items-center justify-between text-xs font-bold text-slate-400 uppercase tracking-wider">
            <span className="flex items-center gap-1.5">
              <Flame className="w-3.5 h-3.5 text-rose-400" /> Zonas Frecuencia Cardíaca
            </span>
          </div>
          ...
          <div className="space-y-2.5">
            {zones.map((zone, index) => {
              ...
              const percentage = totalZoneTime > 0 ? Math.round((time / totalZoneTime) * 100) : 0
              const cfg = ZONE_CONFIGS[index] || { name: `Z${index + 1}`, color: "bg-orange-500", text: "text-orange-400" }
                ...
                <div key={index} className="space-y-1">
                  <div className="flex items-center justify-between text-[11px] font-medium">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-slate-300 font-semibold">{cfg.name}</span>
                      <span className="text-slate-500 font-mono">
                        ({zone.min ?? "?"}–{zone.max ?? "?"} bpm)
                      </span>
                    </div>
                    <div className="flex items-center gap-2 font-mono">
                      <span className={`font-bold ${cfg.text}`}>{percentage}%</span>
                      <span className="text-slate-400">
                        {Math.floor(time / 60)}:{String(Math.round(time % 60)).padStart(2, "0")}
                      </span>
                    </div>
                  </div>
                  <div className="w-full h-2 bg-slate-950 rounded-full overflow-hidden p-0.5 border border-slate-800/50">
                    <div
                      ...
                      style={{ width: `${Math.max(percentage, time > 0 ? 3 : 0)}%` }}
                    />
                  </div>
                </div>
              )
