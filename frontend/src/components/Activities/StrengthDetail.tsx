...
function SetTypeBadge({ setType }: { setType: string | undefined }) {
  switch (setType) {
    case "warm_up":
      return (
        <span className="inline-flex items-center px-2 py-0.5 rounded-md bg-surface-container-high text-muted-foreground text-[10px] font-semibold">
          Calentamiento
        </span>
      )
    case "failure":
      return (
        <span className="inline-flex items-center px-2 py-0.5 rounded-md bg-destructive/20 text-destructive text-[10px] font-bold">
          Fallo
        </span>
      )
    case "drop_set":
      return (
        <span className="inline-flex items-center px-2 py-0.5 rounded-md bg-primary/20 text-primary text-[10px] font-bold">
          Drop set
        </span>
      )
    default:
...
export function StrengthDetail({
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {/* Exercises Cards */}
      {exercises.length === 0 ? (
        <div className="p-6 bg-card/80 border border-border rounded-2xl text-center text-xs font-medium text-muted-foreground">
        ...
        <div className="space-y-4">
          {exercises.map((exercise, exerciseIndex) => {
            ...
            return (
              <div
                key={exerciseIndex}
                className="bg-card/80 border border-border rounded-2xl overflow-hidden shadow-card"
              >
                <div className="bg-card px-4 py-3 border-b border-border flex items-center justify-between flex-wrap gap-2">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {record && Number(record.max_1rm) > 0 ? (
                      <span className="px-2 py-0.5 rounded-lg bg-primary/15 border border-primary/30 text-primary text-[11px] font-bold flex items-center gap-1">
                ...
                <div className="divide-y divide-border/40">
                  {(exercise.sets ?? []).map((set, setIndex) => (
                    <div
                      key={setIndex}
                      className="flex items-center justify-between px-4 py-2 hover:bg-surface-container-high/20 transition-colors"
                    >
                      <SetTypeBadge setType={set.set_type} />
                      <div className="flex items-center gap-3 font-display text-xs">
